    ...
    */
    ...
    public function connect()
    {
        ...
        if(isset($config['hostnames'])) {
            if(isset($config['port'])) {
                $config['hostnames'] .= ':' . $config['port'];
            }
        }else {
            $config['hostnames'] = array(
    ...
    protected function _call ($command, array $arguments = array(), array $values = null)
    {
        ...
        if (isset($collection_name)) {
            $conf = $this->_config['connection'] ?? [];
            if(!empty($conf['table_prefix']) && !str_starts_with($collection_name, $conf['table_prefix'])) {
                $collection_name = $conf['table_prefix'] . $collection_name;
            }
            if(!empty($options['TableSuffix']) && !str_ends_with($collection_name, $options['TableSuffix'])) {
                $collection_name = $collection_name . $options['TableSuffix'];
            }
        ...
        switch ($command) {
            ...
            case 'group':
                ...
                break;
            case 'update':
                $c->update($criteria, $values, $options);
                if(isset($options['ReturnValues']) && $options['ReturnValues']) {
                    $r = $c->findOne($criteria);
                }else {
                    $r = array_merge(...array_values($values));
                }
                break;
            case 'insert':
